...
def _compose_paths(cfg: Dict[str, Any], which: str) -> tuple[Path, Path]:
    ...
    dst = path.with_suffix('.parquet')
    return path, dst


def convert_csv_to_parquet(src_file: Path, dst_file: Path, selected_periods: Optional[list[str]] = None):
...
if __name__ == '__main__':
    ...
    convert_csv_to_parquet(src, dst, args.periods)
    other = 'slim' if args.which == 'full' else 'full'
    try:
        o_src, o_dst = _compose_paths(cfg, other)
        print(f"ℹ️ 另外一个输出: {other}: {o_src} -> {o_dst}")
    except Exception:
        pass
