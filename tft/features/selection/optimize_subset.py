...
def _eval_subset(
    ...
    for t in targets:
        if t not in tr.columns:
            continue
        is_cls = is_classification_target(t)
        if is_cls:
            ytr = tr[t].fillna(0).clip(0, 1).astype(int)
            yva = va[t].fillna(0).clip(0, 1).astype(int)
            ...
            ytr = tr[t].astype(float)
            yva = va[t].astype(float)
...
def rfe_search(
    ...
    while improved and len(keep) > 1:
        ...
        candidates = [cols[i] for i in order[:step]]

        trial_best_score = best_score
        trial_best_keep = keep
        trial_best_detail = best_detail
        for c in candidates:
...
def ga_search(
    ...
    def tournament(pop: List[List[int]], k: int = 3) -> List[int]:
        ...
        cand.sort(key=lambda x: fitness_cache.get(tuple(x), -1e9), reverse=True)
        return cand[0][:]
    ...
    best_ind = max(pop, key=get_fit)
    best_fit = get_fit(best_ind)
    ...
    for g in range(generations):
        ...
        cur_best = max(pop, key=get_fit)
        cur_fit = get_fit(cur_best)
        if cur_fit > best_fit:
...
def main():
    ...
    ap.add_argument("--pool", type=str, default="configs/selected_features.txt", help="Initial candidate feature list (one per line)")
    ap.add_argument("--core-csv", type=str, default="reports/feature_evidence/aggregated_core.csv", help="Fallback: take topk from core csv")
    ...
    ap.add_argument("--sample-cap", type=int, default=50000, help="Limit training rows for speed")
