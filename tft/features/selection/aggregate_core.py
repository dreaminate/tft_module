...
def main():
    ...
    ap.add_argument("--in", type=str, default="reports/feature_evidence/tree_perm", help="Input dir from tree_perm")
    ap.add_argument("--weights", type=str, default="configs/weights_config.yaml", help="Target weights yaml")
    ap.add_argument("--topk", type=int, default=128, help="Top-K features to select")
    ap.add_argument("--tft-file", type=str, default=None, help="Optional TFT gating CSV with columns [feature, score]")
    ap.add_argument("--tft-bonus", type=float, default=0.0, help="Bonus weight added from TFT gating (0..1)")
    ...
    df = aggregate_tree_perm(args.__dict__["in"])  # avoid Python keyword in
