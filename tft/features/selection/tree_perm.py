...
def fit_tree_and_permutation(
    ...
    if is_cls:
        ...
        perm = permutation_importance(
            clf, X_va, y_va_bin, scoring="f1", n_repeats=10, n_jobs=-1, random_state=random_state
        )
        ...
        perm = permutation_importance(
            reg, X_va, y_va, scoring="neg_root_mean_squared_error", n_repeats=10, n_jobs=-1, random_state=random_state
        )
        return reg.feature_importances_, perm.importances_mean, perm.importances_std, float(base_score)


def run(periods: List[str],
        val_mode: str,
        val_days: int,
        val_ratio: float,
        topn_preview: int,
    ...
    for per in periods:
        ...
        for target in ds.targets:
            if target not in per_tr.columns:
                print(f"[warn] target={target} not in data; skip")
                continue
            ...
            X_tr, y_tr, X_va, y_va, med = _prep_xy(per_tr, per_va, ds.features, target)
...
def main():
    ...
    ap.add_argument("--val-mode", type=str, default="ratio", choices=["ratio","days"], help="Validation split mode")
    ap.add_argument("--val-days", type=int, default=30, help="If val-mode=days, how many days for validation")
    ap.add_argument("--val-ratio", type=float, default=0.2, help="If val-mode=ratio, validation ratio")
    ap.add_argument("--preview", type=int, default=10, help="Print top-N for quick preview")
    ap.add_argument("--out", type=str, default="reports/feature_evidence/tree_perm", help="Output directory")
    args = ap.parse_args()
