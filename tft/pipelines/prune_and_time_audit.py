        ...
        if (step_use is None) or (step_use <= 0) or (unique_ts == 1):
            ...
            for m in list_missing(ts, step_use, limit_points=2000):
                gaps_rows.append({
                    ...
                    'period': (gkey[1] if has_symbol and has_period else (gkey if (has_period and not has_symbol) else None)) if group_cols[0] != '__dummy__' else None,
                    'missing_ts': int(m),
                    'missing_dt': pd.to_datetime(int(m), unit='ms', utc=True)
                })
...
def process_one_file(in_path: Path, out_path: Path, how: str, *, min_k=None, min_ratio=None, filter_periods=None, report_dir: Path | None = None, diagnose: bool = True):
    ...
    if report_dir:
        ...
        if diagnose and n_filter_cols:
            cov, cov_by_p = diagnose_missing_coverage(raw, ROW_FILTER_COLS, in_path.name)
            if not cov.empty:
                cov.to_csv(report_dir / (in_path.stem + '.missing_coverage.csv'), index=False)
            if not cov_by_p.empty:
                cov_by_p.to_csv(report_dir / (in_path.stem + '.missing_coverage_by_period.csv'), index=False)
